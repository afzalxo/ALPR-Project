...
if __name__ == '__main__':
    ...
    for wo in range(len(img_paths)):
        ...
        img_name = os.path.join('../ccpd/CCPD2019', img_name).strip()
        ...
        if iou >= 0.7:
            correct_pred += 1
        #print('IoU: %s' % str(iou))
        print('Current Accuracy: %s' % str(correct_pred/(wo+1)))
        ...
        for i in range(bboxes.shape[0]):
            ...
            cv2.imwrite('det_results/base_eval/cropped/' + str(real_img_n.strip()), cropp)
            cv2.rectangle(image, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), (0, 0, 255), 2)
        image = cv2.resize(image, (0, 0), fx = 1/args.scale, fy = 1/args.scale, interpolation=cv2.INTER_CUBIC)
        cv2.imwrite('det_results/base_eval/full_images/' + str(real_img_n.strip()), image)
